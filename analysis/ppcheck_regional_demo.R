...
ppcheck_yayoi <- ppcheck_1000yrs <- vector('list',length=nrow(regionStartDates))
...
for (i in 1:nrow(regionStartDates))
{
	...
	params <- list(r=post.sample.1000yrs[[1]][,paste0('r[',i,']')])
	ppcheck_1000yrs[[i]] <- postPredSPD(obs.cra,errors=obs.error,calCurve = 'intcal20',model = dExponentialGrowth,a = a,b=b,params=params,method='uncalsample',nsim = 500,ncores = 3,verbose=TRUE)
...
save(ppcheck_1000yrs,file=here('R_images','ppcheck_1000yrs.RData'))
