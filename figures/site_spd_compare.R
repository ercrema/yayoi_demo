...
for (k in 1:8)
{
	...
	spdMats[[k]]  <- spd(calibrated.dates[j],timeRange=c(a,b),spdnormalised=T)
	spdMats.n[[k]]  <- spd(calibrated.dates.n[j],timeRange=c(a,b),spdnormalised=T)
}



pdf(file=here('figures','site_spd_compare.pdf'),width=5.3,height=2.6,pointsize=2)
cex=0.9
...
for (i in 1:8)
{
	...
	plot(NULL,xlim=c(a,b),ylim=c(0,maxSPD),axes=F,xlab='BC/AD',ylab='Summed Probability',yaxs='i',xaxs='i',cex.main=cex,cex.axis=cex,cex.lab=cex,cex=cex)
	...
	polygon(c(a:b,b:a),c(spd2[,2],rep(0,nrow(spd2))),border=NA,col='lightgrey',cex=cex)
	lines(a:b,spd1[,2],lty=1,cex=cex)
	par(new=T)
	plot(NULL,xlim=c(a,b),ylim=c(0,max(kdesites[,-1])),axes=F,xlab='',ylab='',yaxs='i',xaxs='i',cex.main=cex,cex.axis=cex,cex.lab=cex,cex=cex)
	...
	axis(3,at=c(0,-500,-750,-1000)+BCADtoBP(ricearrival$m[i]),labels=c(0,500,750,1000),padj=+.5,cex.axis=cex,cex.lab=cex)
