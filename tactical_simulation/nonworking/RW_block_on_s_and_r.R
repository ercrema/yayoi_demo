...
model <- nimbleCode({
  ...
  rhosq ~ dexp(1000);
  mu_s[1:N] <- 0;
  cov_s[1:N, 1:N] <- cov_GPL2(dists[1:N, 1:N], rhosq, etasq, 0.0000001)
...
conf$addSampler(c('r','s[1:500]'), type='RW_block') 

MCMC <- buildMCMC(conf)
cMCMC <- compileNimble(MCMC)
results <- runMCMC(cMCMC, inits=inits,nchain=1,niter = 100000, thin=5,nburnin = 50000, progressBar = TRUE)
save(results,file=here('tactical_simulation','rw_sr_res.RData')) 